...
ADMISSION_KB = {
 "btech_fees": "B.Tech fees: Rs 1,50,000 per year. Hostel: Rs 60,000 per year.",
...
HOSTEL_KB = {
 "availability": "Boys: 4000 seats. Girls: 3500 seats. Apply early.",
 "room_types": "Single: Rs 80,000/yr. Double: Rs 60,000/yr. Triple: Rs 50,000/yr.",
 "rules": "Curfew: 10 PM. No visitors after 8 PM. Ragging strictly prohibited.",
 "mess": "Veg and non-veg options. Meal timings: 7AM, 1PM, 7PM. Monthly pass: Rs3,500.",
 "facilities": "24x7 Wi-Fi, gym, common room, laundry service, medical center.",
}
...
ACADEMIC_KB = {
 ...
 "credits": "B.Tech total: 160 credits. Each subject: 3-4 credits.",
 ...
 "backlog": "Failed subjects can be cleared in supplementary exams or re-registration.",
}
...
def classify_query(state: UniversityState)->UniversityState:
  """
  Classifies query into one of 4 domains.
  """
  ...
  prompt=(
    ...
    "-hostel: rooms, curfew, facilities, hostel rules\n"
...
def _agent_respond(state:UniversityState, knowledge_base:dict, domain_name:str)-> UniversityState:
  ...
  for key, value in knowledge_base.items():
    ...
    # creates relevant entries based on words
  ...
  if state.get("cgpa"):
    student_info += "CGPA: " + str(state["cgpa"]) + "\n"
...
# **state -> unpacks state and then updates message field
...
def build_university_graph(db_path: str):
  ...
  memory = SqliteSaver(conn) # creates a langgraph checkpointer that uses db connection

  return builder.compile(checkpointer=memory)
  # load the saved state for user_001 from SQLite
